...
export function Navbar() {
    ...
    return (
        <header className="sticky top-0 z-50 w-full border-b border-white/5 bg-background/80 backdrop-blur-md">
            ...
            {/* Mobile Menu */}
            {isMenuOpen && (
                <div className="md:hidden absolute top-16 left-0 w-full border-b border-white/5 bg-background p-4 flex flex-col gap-4 animate-in slide-in-from-top-2 shadow-xl z-50">
                    {navItems.map((item) => (
                        <Link
                            key={item.path}
                            to={item.path}
